...
vec3 grainField(vec2 uv, float t) {
  ...
  vec3 color = mix(vec3(0.0824), vec3(0.1373, 0.1412, 0.1216), smoothstep(0.0, 0.42, bands));
  color = mix(color, vec3(0.2275, 0.2667, 0.2667), smoothstep(0.38, 0.7, bands) * 0.72);
  color = mix(color, vec3(0.5020), smoothstep(0.66, 0.88, bands) * 0.48);
  color = mix(color, vec3(0.8980, 0.9020, 0.8824), smoothstep(0.84, 1.0, bands) * 0.34);
