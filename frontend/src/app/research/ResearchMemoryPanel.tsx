...
export function ResearchMemoryPanel({ taskId, projectName, preview = false }: { taskId: string | null; projectName?: string; preview?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!editor || busy || !limits || !content.trim()) return
  ...
  const editorForm = editor ? <form className="research-memory__editor" onSubmit={event => void submit(event)}>
      ...
      <textarea ref={editorRef} id="memory-content" rows={4} value={content} onChange={event => setContent(event.target.value)} aria-describedby="memory-content-budget" aria-invalid={contentTooLong} placeholder="例如：比较不同解释时，先回到原始材料核验。" disabled={busy} required />
      <p id="memory-content-budget" className={contentTooLong ? 'research-memory__error' : 'research-memory__notice'}>本条 {contentBytes} / {limits?.max_content_bytes ?? '…'} 字节（UTF-8）{contentTooLong ? '，请缩短内容后保存。' : '；通常一个汉字占 3 字节。'}</p>
      <footer><span>{taskId ? '保存到项目记忆' : '保存到个人记忆'}</span><button type="button" disabled={busy} onClick={() => setEditor(null)}>取消</button><button className="research-memory__primary" type="submit" disabled={busy || !limits || !content.trim() || contentTooLong || (editor === 'new' && atCapacity)}>{busy ? '保存中…' : '保存记忆'}</button></footer>
    </form> : null
  ...
  return <div className="research-memory">
    <section className="research-memory__overview" aria-label="记忆概览" aria-busy={loading || summaryBusy}>
      ...
      <footer className="research-memory__overview-footer">
        ...
        <button type="button" disabled={loading || busy || !settings || !limits || atCapacity} onClick={() => edit('new')}><PlusIcon size={15} />添加记忆</button>
        {!preview && summary ? <span>根据已保存的记忆整理，仅供阅读；Agent 每轮按需参考。</span> : null}
      </footer>
      {atCapacity ? <p className="research-memory__notice">已达到 {limits?.max_entries} 条上限，可编辑已有记忆，或删除后再添加。</p> : null}
      {taskId ? <p className="research-memory__notice">项目对话也会参考已开启的个人记忆。</p> : null}
    ...
    {settingsOpen ? <section className="research-memory__settings" id="memory-settings" aria-label="记忆设置">
      <div><div><strong>使用{taskId ? '项目' : '个人'}记忆</strong><p>{taskId ? '在当前项目的对话中按需参考这些记忆。' : '在对话中按需参考你的个人记忆。'}关闭后保留内容，不会同时关闭学习。</p></div><button type="button" role="switch" aria-label={`使用${taskId ? '项目' : '个人'}记忆`} aria-checked={settings?.use_memory ?? false} disabled={!settings || busy} onClick={() => void toggle('use_memory')} /></div>
      <div><div><strong>从对话中学习</strong><p>对话闲置后，后台整理值得保留的信息。此开关独立控制学习；关闭后仍可手动添加或在对话中明确要求记住。</p></div><button type="button" role="switch" aria-label="从对话中学习" aria-checked={settings?.learn_memory ?? false} disabled={!settings || busy} onClick={() => void toggle('learn_memory')} /></div>
    </section> : null}
    ...
    <p className="research-memory__notice">手动添加、修改或明确要求记住的内容，后台学习不会覆盖；这不代表每轮对话都会加载全文。</p>
