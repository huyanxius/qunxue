...
export function CourseWorkshop({ templateId, courseTitle, onTeaching, expanded = false }: { templateId?: string; courseTitle: string; onTeaching?: () => void; expanded?: boolean }) {
  ...
  return <section className="course-workshop" aria-label="集训安排与成果要求">
    ...
    <div className="course-workshop__stages">{plan.stages.map((stage, index) => <details key={stage.title} open={expanded || index === 0}>
