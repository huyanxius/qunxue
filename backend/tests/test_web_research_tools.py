...
from qunxue_api.adapters.research_agent.web_research import (
...
def test_web_search_does_not_retry_the_same_query_inside_one_tool_scope() -> None:
    tools = KnowledgeToolRegistry(_Catalog(), web_research=_WebResearch())
    tools.enable_web_search()

    first = tools.search_web("青年就业 政策", limit=3)
    second = tools.search_web("青年就业 政策", limit=3)
...
def test_open_web_client_uses_explicit_custom_json_provider() -> None:
    ...
    assert parse_qs(urlparse(request_url).query)["q"] == ["青年就业 政策"]
    assert parse_qs(urlparse(request_url).query)["limit"] == ["8"]
    ...
    class Reranker:
        def search_chunks(self, *, query, chunks, limit):
            ...
            return SimpleNamespace(
                hits=tuple(
                    SimpleNamespace(chunk=by_id[chunk_id]) for chunk_id in ("web:0", "web:1")
                )
            )

    client = OpenWebResearchClient(search=search, profile="generic", reranker=Reranker())
