...
def test_deep_report_continues_when_provider_truncates_output():
    r = runner()
    calls = []
    ...
    def model(messages, info):
        ...
        if len(calls) == 1:
            return ModelResponse(parts=[TextPart("证据已核验")])
        if len(calls) == 2:
            return ModelResponse(parts=[TextPart("研究结论：")], finish_reason="length")
        return ModelResponse(parts=[TextPart("证据仍有边界。")], finish_reason="stop")

    # Non-streaming responses carry the provider finish reason; the same helper
    # must preserve all text when the UI supplied an event handler.
    with r._agent.override(model=FunctionModel(model)):
        result = r.run(prompt="已确认提案", conversation=(), tools=tools())
    assert result.answer == "研究结论：证据仍有边界。"
    assert all(c["max_tokens"] >= 12000 for c in calls)


def test_repeated_output_truncation_is_interrupted_not_completed():
    ...
    def model(messages, info):
        calls.append(1)
        return ModelResponse(parts=[TextPart("尚未写完")],
                             finish_reason="stop" if len(calls) == 1 else "length")

    with r._agent.override(model=FunctionModel(model)), pytest.raises(AgentInterrupted):
        r.run(prompt="已确认提案", conversation=(), tools=tools())
    assert len(calls) == 4
