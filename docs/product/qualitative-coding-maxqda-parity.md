...
## 10. 像素级视觉逆向规格（MAXQDA Document Browser）

本节以官方原始截图 `05-EN-CodeVisualWindow.png`（2000 × 998）为测量基准。数值是界面实现的初始锁定值，浏览器适配只允许按比例缩放，不允许改变层级和相对位置。

### 10.1 画布分区

| 区域 | 基准位置/尺寸 | 实现要求 |
| --- | --- | --- |
| 文档身份行 | y=0–72 | 左侧文件图标、文件名、行数；右侧搜索、缩放、打印、导出和设置 |
| 编码工具行 | y=72–142 | 文档路径下拉、编码动作组、颜色编码组、memo/评论动作 |
| 编码页边 | x≈0–410 | 三点设置、代码名、彩色竖条/括号、memo 图标 |
| 行号列 | x≈410–458 | 固定宽度、浅灰底、蓝灰行号，不能随正文选择移动 |
| 正文列 | x≈458–2000 | 连续文本流；编码高亮只覆盖字符，不改变换行 |
| 右侧工具 | x≈1880–2000 | 侧栏开关、设置、折叠等图标，不侵入正文 |

### 10.2 视觉令牌

- 文档背景：`#f7f7f7` 左右，正文纸面不使用渐变；
- 正文文字：近黑 `#111111`，行高约 1.55，段落间距 8–14px；
- 行号：蓝灰 `#55718f`，字号约为正文的 0.72；
- 编码高亮：低饱和半透明色，默认只显示当前激活代码；
- 编码条：4–6px 宽，条与条之间 10–16px；括号首尾必须与编码范围对齐；
- 代码名：12–14px sans-serif，颜色与编码条一致，超长省略但 tooltip 显示完整父子路径；
- 工具栏：32–40px 控件高度，图标按钮 32px 方形，组间 16px 分隔；
- 选中态：不使用强阴影；使用 1px 深色边框 + 低对比度底色；
- 候选态：编码条使用虚线或点线，正文高亮透明度低于确认编码；
- 重叠态：编码条平行排列，正文不叠加混色。

### 10.3 工具栏顺序与语义

工具栏必须保持以下顺序，不能按实现方便随意排列：
...
每个按钮都必须有：可见图标、中文 tooltip、键盘焦点、禁用原因。只有当前有文本选区时，依赖选区的编码按钮才可用；没有选区时不能点击后静默失败。

### 10.4 交互契约

- 单击正文：定位/选中片段，不打开新页面；
- 拖选正文：保留浏览器选区，弹出编码动作，不抢走选区；
- 单击编码条：选中该代码和片段，右侧显示检查器；
- 悬停编码条：对应代码名和整条范围高亮，其他条降低对比度；
- 双击编码条：打开该片段评论编辑器；
- 单击 memo 图标：在侧栏打开 memo；双击进入编辑；
- 单击检索结果来源：回到原文并将片段滚动到视口中央；
- `Esc`：依次关闭菜单、检查器、选区动作层，不离开材料；
- `⌘/Ctrl + 1…9`：将选中片段快速赋给已绑定代码；
- 右键正文或编码条：打开上下文菜单，菜单项按当前对象变化；
- 改变筛选：只改变可见编码和结果，不改变正文、选区或历史定位；
- 切换材料/解析版本：清空不属于新材料的选中状态，并明确显示历史版本标识。
...
检查器打开后采用独立列，不覆盖正文；窄屏才改为抽屉。关闭检查器后，正文滚动位置和选区必须恢复。

### 10.6 与现有实现的差距清单

当前实现已经具备 segment 级编码条、基础筛选和证据检查器，但仍缺少：

- 固定三栏文档浏览器布局；
- 完整 MAXQDA 顺序的工具栏和上下文菜单；
- 字符级编码范围及多范围重叠渲染；
- 代码树激活状态与频次；
- memo/评论独立列；
- Retrieved Segments 表格/列表双视图；
- PDF/DOCX/XLSX/HTML 编码文档导出；
- 快捷键绑定、历史版本和审计回放。

这些项目全部完成并完成实际浏览器视觉检查后，才允许称为最终版。
