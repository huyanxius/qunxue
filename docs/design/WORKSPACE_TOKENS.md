...
用户要求采用现有界面偏暖的颜色。提取 `AppFrameShader.tsx` 的暖白 `#f8f7f2`、沙色 `#c3ad94`、灰绿 `#8f9f97`，在 `tokens.css` 定义 `--qx-paper-*`。设置内容为暖白，侧栏混入少量沙色，选中项按最终验收使用暖灰 `#e9e8e3`，文字用炭灰。仅设置页面启用这组语义映射，其他页面配色不随设置页更改。
