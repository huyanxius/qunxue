...
REFERENCE_RE = re.compile(r"^(?:>\s*)?(?:\*\*)?文献\s*[：:](?:\*\*)?")
SUPPORT_RE = re.compile(r"^(?:>\s*)?(?:\*\*)?支持范围\s*[：:](?:\*\*)?")
...
def build_report(
    ...
    evidence_reports = [
        report for report in reports if report["dimension"] in ("本体论", "认识论", "方法论", "实践论")
    ]
    evidence_summary = "；".join(
        f"{report['dimension']} {report['claim_count']}/{report['reference_count']}/{report['support_count']}"
        for report in evidence_reports
    )
    evidence_gaps = [
        f"{report['dimension']}差{int(report['reference_count']) - int(report['support_count']):+d}"
        for report in evidence_reports
        if report["reference_count"] != report["support_count"]
    ]
    lines += [
        "## 源文档不一致与处理",
        "",
        ...
        f"3. **观点与文献定稿结构**：本体论、认识论、方法论、实践论已替换为用户确认的正确稿。按“观点标题/文献行/支持范围行”统计为：{evidence_summary}。文献与支持范围核对：{'全部相等' if not evidence_gaps else '、'.join(evidence_gaps)}。价值论尚未提供同类定稿版，当前三项仍为 0。",
        f"4. **实践论编号**：正确稿已使用连续唯一的 `P001`–`P370`，转换时调整 {practice_report['normalized_id_count']} 个标题；无需再处理旧稿中的 `R` 前缀。",
        f"5. **认识论拆分**：正确稿经 Pandoc 还原为 {epistemology_report['category_count']} 个大类和 {epistemology_report['subcategory_count']} 个子类，按既定规则生成 {len(epistemology_report['files'])} 个文件，与目录约定 {epistemology_report['expected_files']} 个一致。",
        ...
        f"7. **方法论编号**：正确稿已使用连续唯一的 `M001`–`M690`，转换时调整 {methodology_report['normalized_id_count']} 个标题；无需再处理旧稿中的重复编号。",
